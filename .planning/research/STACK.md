...
**Domain:** Browser-based PDF editor with content-stream text rewriting, font subsetting, and multi-format export
**Researched:** 2026-08-11
**Confidence:** HIGH on engine + fonts + licensing. MEDIUM on export quality. MEDIUM on client-side split.

---

## The One Finding That Drives Everything

**No permissively-licensed library does both halves of content-stream text editing.** You need three
libraries, not one:
...
The single library that does all three is **MuPDF/PyMuPDF — and it is AGPL, which triggers on SaaS.**
That is the central licensing fact of this project. See "The AGPL Trap" below.

This is not theoretical: [`pdftopdfa` 0.9.0](https://pypi.org/project/pdftopdfa/) (MPL-2.0) ships a
production PDF/A converter built on exactly `pikepdf 10.9+` + `fontTools 4.63+` + `pdfminer.six`, with
Ghostscript explicitly removed. That is a working proof of this stack shape.
...
All are OFL/permissive — embedding subsets in user output is explicitly allowed.
...
```bash
# Engine (Python 3.13)
uv add pikepdf playa-pdf fonttools uharfbuzz pypdfium2 pillow python-docx
...
# Client
npm install pdfjs-dist @cantoo/pdf-lib react react-dom
npm install -D vite typescript @vitejs/plugin-react

# System packages (Dockerfile) — subprocess only, never linked
apt-get install -y libreoffice-writer poppler-utils default-jre-headless
...
The distinction the roadmap depends on. "Append" libraries can only *add* drawing operators to a page;
they cannot locate and change existing ones. Every white-box-overlay PDF editor exists because its
author reached for an append-only library.
...
**Second capability table — who decodes font encodings?** (pikepdf does *not*; it hands you raw
dictionaries. This is why you need two libraries on the read side.)
...
**Artifex dual-licenses MuPDF *and* Ghostscript under AGPL-3.0 or a paid commercial license.** The AGPL's
network clause means that *making the software available as SaaS counts as conveying it* — you must
release your entire server source under AGPL, or buy a license.

Artifex is explicit about this: *"The moment you pass that software to any other person, or make it
available to any other person as part of a 'Software as a service' installation, you must abide by the
AGPL terms."* Commercial licensing reportedly starts around $1,500 and scales past $50,000.

This rules out, for a commercial-intent product:
...
**GPL is a different and much smaller problem.** GPL/LGPL obligations trigger on *distribution*, not on
network use. A SaaS that shells out to `pdftohtml` or `veraPDF` distributes nothing, so poppler (GPL) and
veraPDF (GPL-3.0/MPL-2.0) are safe as subprocesses. **Only AGPL follows you over the network.** Do not
let a blanket "no copyleft" rule cost you poppler; do not let a loose one cost you the company.

Everything recommended above is MPL-2.0, MIT, Apache-2.0, BSD, or GPL-as-subprocess. Zero AGPL.
...
**If the user's session contains only page operations (insert / reorder / rotate / delete / merge):**
...
**If the session contains any text edit, font change, or format conversion:**
...
**If a conversion consistently exceeds ~60s (realistically: LibreOffice-backed paths only):**
...
**If PDF/A output must be certified rather than best-effort:**
...
**Live registry queries (2026-08-11)** — PyPI, npm, crates.io, GitHub API for every version and license stated above.
...
---
*Stack research for: browser-based PDF editor with content-stream text editing*
*Researched: 2026-08-11*
